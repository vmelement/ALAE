...
from model_metric import Model
...
def train(cfg, logger, local_rank, world_size, distributed):
    ...
    model = Model(
        ...
        metric_start_lod=cfg.MODEL.METRIC_START_LOD
    )
    ...
    # lfj
    dataset = TFRecordsDataset(cfg, logger, rank=local_rank, world_size=world_size, buffer_size_mb=1024, channels=cfg.MODEL.CHANNELS, metric=True)
    ...
    if cfg.DATASET.SAMPLES_PATH:
        ...
        sample_x, sample_x2, sample_y = next(make_dataloader_xy(cfg, logger, dataset, 32, local_rank))
        sample = (sample_x / 127.5 - 1.)
        sample_x2 = (sample_x2 / 127.5 - 1.)
    ...
    for epoch in range(scheduler.start_epoch(), cfg.TRAIN.TRAIN_EPOCHS):
        ...
        batches = make_dataloader_xy(cfg, logger, dataset, lod2batch.get_per_GPU_batch_size(), local_rank)
        ...
        i = 0
        for x_orig, x2_orig, y_orig in tqdm(batches):
            i += 1
            with torch.no_grad():
                ...
                if need_permute:
                    ...
                    x2_orig = x2_orig.permute(0, 3, 1, 2)
                x_orig = (x_orig / 127.5 - 1.)
                x2_orig = (x2_orig / 127.5 - 1.)

                blend_factor = lod2batch.get_blend_factor()
                # TODO: do we need to bump up resolution of x2
            ...
            # Do we need x2 to set requires_grad to True
            x.requires_grad = True

            encoder_optimizer.zero_grad()
            loss_d = model(x, x2, y_orig, lod2batch.lod, blend_factor, d_train=True, ae=False)
            tracker.update(dict(loss_d=loss_d))
            loss_d.backward()
            encoder_optimizer.step()

            decoder_optimizer.zero_grad()
            loss_g = model(x, x2, y_orig, lod2batch.lod, blend_factor, d_train=False, ae=False)
            ...
            encoder_optimizer.zero_grad()
            decoder_optimizer.zero_grad()
            lae = model(x, x2, y_orig, lod2batch.lod, blend_factor, d_train=True, ae=True)
...
if __name__ == "__main__":
    ...
    run(train, get_cfg_defaults(), description='StyleGAN', default_config='configs/vgg2_metric.yaml', world_size=4)
