...
def train(cfg, logger, local_rank, world_size, distributed):
    ...
    model = Model(
        ...
        identity_count = cfg.MODEL.IDENTITY_COUNT,
        arcface_start_lod = cfg.MODEL.ARCFACE_START_LOD
    )
    ...
    # lfj
    dataset = TFRecordsDataset(cfg, logger, rank=local_rank, world_size=world_size, buffer_size_mb=1024, channels=cfg.MODEL.CHANNELS, needs_labels=True)
    ...
    if cfg.DATASET.SAMPLES_PATH:
        ...
        sample_x, sample_y = next(make_dataloader_y(cfg, logger, dataset, 32, local_rank))
    ...
    for epoch in range(scheduler.start_epoch(), cfg.TRAIN.TRAIN_EPOCHS):
        ...
        batches = make_dataloader_y(cfg, logger, dataset, lod2batch.get_per_GPU_batch_size(), local_rank)
        ...
        i = 0
        for x_orig, y_orig in tqdm(batches):
            i += 1
            with torch.no_grad():
            ...
            loss_d = model(x, y_orig, lod2batch.lod, blend_factor, d_train=True, ae=False)
            tracker.update(dict(loss_d=loss_d))
            loss_d.backward()
            encoder_optimizer.step()

            decoder_optimizer.zero_grad()
            loss_g = model(x, y_orig, lod2batch.lod, blend_factor, d_train=False, ae=False)
            ...
            encoder_optimizer.zero_grad()
            decoder_optimizer.zero_grad()
            lae = model(x, y_orig, lod2batch.lod, blend_factor, d_train=True, ae=True)
...
if __name__ == "__main__":
    ...
    run(train, get_cfg_defaults(), description='StyleGAN', default_config='configs/vgg2.yaml', world_size=4)
