...
class Model(nn.Module):
    def __init__(self, startf=32, maxf=256, layer_count=3, latent_size=128, mapping_layers=5, dlatent_avg_beta=None,
                 ...
                 encoder="", z_regression=False, metric_start_lod=4):
        super(Model, self).__init__()

        self.layer_count = layer_count
        self.z_regression = z_regression
        self.metric_start_lod = metric_start_lod
    ...
    def forward(self, x, x2, y, lod, blend_factor, d_train, ae):
        if ae:
            ...
            # compute metric loss when image is large enough
            if lod >= self.metric_start_lod:
                embed_real2, d_res_real2 = self.encoder(x2, load, blend_factor)
                # loss_metric = None
                pass
